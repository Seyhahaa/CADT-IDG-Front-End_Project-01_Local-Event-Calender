    ...
    <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div class="relative overflow-hidden rounded-lg bg-white px-4 pb-12 pt-5 shadow sm:px-6 sm:pt-6">
        <dt>
          ...
          <p class="ml-16 truncate text-sm font-medium text-gray-500">Recent Activities</p>
        </dt>
        <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
          ...
          <tbody class="divide-y divide-gray-200 bg-white">
            <tr v-for="event in recentEvents" :key="event.id">
              ...
              <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                <span :class="[
                  event.status === 'Upcoming' ? 'text-green-700 bg-green-50' : 'text-gray-700 bg-gray-50',
              ...
              <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                <RouterLink :to="`/user/events/${event.id}`" class="text-indigo-600 hover:text-indigo-900">
                  View
                </RouterLink>
              </td>
            </tr>
