<template>
    ...
    <!-- Event Content -->
    <div v-else>
        <h1 class="mb-5 font-bold text-2xl">Update News</h1>
        <!-- Event Header -->

        <section class="bg-gray-100">
            <div class="mx-auto max-w-screen-xl px-4 py-6 sm:px-6 lg:px-6">
                <Form @submit="updateNew" class="space-y-4">
...
export default {
    ...
    methods: {
        async updateNew() {
            ...
            try{
            const response = await axios.put(`${process.env.VUE_APP_SERVER}/news/update/${this.newsId}`,formData,
                {
                    headers: {
        ...
        async getData() {
            //console.log(eventId);
            const response = await axios.get(`${process.env.VUE_APP_SERVER}/news/${this.newsId}`)
