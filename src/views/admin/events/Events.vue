        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="event in paginatedEvents" :key="event.id" class="hover:bg-gray-50">
            <td class="px-6 py-4" @click.stop>
              <input type="checkbox" v-model="selectedEvents" :value="event.id" class="rounded" />
            </td>
            <td class="px-6 py-4 cursor-pointer" @click="router.push(`/admin/events/${event.id}`)">
              <div class="flex items-center">
                <img :src="event.image" class="h-10 w-10 rounded-lg object-cover mr-3" />
                <div>
            ...
            <td class="px-6 py-4 text-sm font-medium" @click.stop>
              <div class="flex space-x-3">
                ...
                <button @click="confirmDelete(event)" class="text-red-600 hover:text-red-900">
                  Delete
                </button>
              </div>
            </td>
...
const filteredEvents = computed(() => {
  return events.value.filter((event) => {
    ...
    // Category filter
    const matchesCategory = !filters.value.category || event.category === filters.value.category

    // Date range filter
    const eventDate = new Date(event.date)
    const matchesDateFrom = !filters.value.dateFrom || eventDate >= new Date(filters.value.dateFrom)
    const matchesDateTo = !filters.value.dateTo || eventDate <= new Date(filters.value.dateTo)

    // Status filter
    const matchesStatus = !filters.value.status || event.status === filters.value.status

    return matchesSearch && matchesCategory && matchesDateFrom && matchesDateTo && matchesStatus
  })
})
